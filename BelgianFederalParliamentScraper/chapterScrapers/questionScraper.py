...
class questionScraper(chapterScraper):
    def __init__(self):
        ...
        self.cleanupRegex = "[0-9][0-9]|-|Question de|Vraag van|mevrouw |de heer |Mme |M. "
    def changeOldCode(self,text:str):
        #assumes old code is present
        newcode = re.search(self.codeRegexPre55,text).group()[1:-1]
        newcode = str(self.legislature) +re.search("[0-9]+",newcode).group().zfill(6) + "P"
        return newcode
    ...
    def isQuestion(self,element:scrapy.selector.unified.Selector)->bool:
        ...
        #in: element
        #out: boolean
        isTitle = bool(re.search("titre|titel",self.getClass(element),flags=re.IGNORECASE))
    ...
    def getCode(self,text:str)->str: #assumes code exsists
        ...
        if out == None:
            return None
        #print("getting code from",text," found code",re.sub("\(|\)","",out.group()))
        return re.sub("\(|\)","",out.group())
        #return out.group()[1:-1]
    ...
    def getTopics(self,elements:list)->pd.DataFrame:
        ...
        for i in range(0,len(elements)):
            ...
            if self.isQuestion(element):
                questions = questions.append({"index":i,"content":self.getInnerText(element),"language":self.getLanguageFromElement(element),"code":self.getCode(self.getInnerText(element))},ignore_index=True)
        return questions
        ...
        try:
            ...
            for blockid in startStopIndexes:
                if startStopIndexes[blockid][1] == None:
                    startStopIndexes[blockid] = (startStopIndexes[blockid][0],len(elements))

                stopIndex = startStopIndexes[blockid][0] + self.findEndOfBlock(elements[startStopIndexes[blockid][0]:startStopIndexes[blockid][1]])
                
                debate = self.scrapeDebate(elements[startStopIndexes[blockid][0]:stopIndex])
            ...
            self.questions["asker"] = self.questions.apply(self.extractQuestionAsker,axis=1)
            self.questions["interviewee"] = self.questions.apply(self.extractInterviewee,axis=1)
            self.mergedQuestions = self.mergeTopics(self.questions,["asker","interviewee","blockid"],["index"],"code","language")
            #print(self.answers["speaker"].unique())
    ...
    def getBlockIndices(self):
        out = self.mergeTopics(self.questions,["asker","interviewee","blockid","index"],[],"code","language")
