...
class legislationScraper(chapterScraper):
    ...
    def findStartArticleDiscussion(self,elements:list)->int:
        for i in range(0,len(elements)):
            if (self.isTitle(self.getClass(elements[i])) and bool(re.search("Discussion des articles|Bespreking van de artikelen|artikel|article",self.getInnerText(elements[i]),flags=re.IGNORECASE))) or bool(re.search("^Discussion des articles|^Bespreking van de artikelen",self.getInnerText(elements[i]),flags=re.IGNORECASE)):
                return i+1
        return len(elements)

    def findStartGeneralDiscussion(self,elements:list)->int:
        for i in range(0,len(elements)):
            if self.isTitle(self.getClass(elements[i])) and bool(re.search("Discussion générale|Algemene bespreking|Algemene|générale",self.getInnerText(elements[i]),flags=re.IGNORECASE)):
                return i+1
        return 0
    ...
    def getTopics(self,elements)->pd.DataFrame:
        ...
        for i in range(0,len(elements)):
            if self.isProposal(elements[i]):
                ...
                topics.append({"index":i,"content":text,"code":self.getTopicCode(text),"language":self.getLanguageFromElement(elements[i])})
    ...
    def parse(self,elements:list)->None:
        ...
        try:
            ...
            startStopIndexes = self.startEndPointOfBlock(self.proposals,blockIdName="blockid")
            ...
            if len(self.debates) >0:
                ...
                self.debates = self.debates.astype({"speechIndex":"int","segmentIndex":"int"})

            if len(self.proposals) ==0:
                raise exceptions.EmptyTopicsDataFrame()
            self.proposals.rename(columns={"code":"fullCode"},inplace=True)
            self.proposals["code"] = self.proposals['fullCode'].apply(self.getDocumentCode) #remove the individual articles
            self.mergedTopics =  self.mergeTopics(self.proposals,singleColumns=["blockid","fullCode"],dropColumns=["index"],matchColumn="code",langColumn="language")
    ...
    def getBlockIndices(self):
        out = self.mergeTopics(self.proposals,singleColumns=["blockid","index"],dropColumns=["fullCode"],matchColumn="code",langColumn="language")
