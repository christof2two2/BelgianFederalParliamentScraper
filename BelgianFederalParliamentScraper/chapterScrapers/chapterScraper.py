...
class chapterScraper(parentClass):
    ...
    def identifyBlocks(
        self, elements: pd.DataFrame, blockIdName="blockid"
    ) -> pd.DataFrame:
    ...
    def startEndPointOfBlock(
        self, blocks: pd.DataFrame, blockIdName: str = "blockid"
    ) -> dict:
    ...
    def getSpeakerName(self, text: str) -> str:
        text = re.sub("\((.*?)\)|\.|\d", "", text)
        text = re.sub(
            "eerste minister|premier|ministre|minister|,|secrétaire d'État|staatssecretaris|première|rapporteur",
            "",
            text,
            flags=re.IGNORECASE,
        )
    ...
    def scrapeDebate(self, elements: list) -> pd.DataFrame:
        ...
        for i in range(begin, len(elements)):
            ...
            if self.newSpeaker(element):
                # add segment to list
                if len(content) > 0:
                    row = self.makeDebateSegmentRow(
                        content, currentSpeaker, currentLang, segmentIndex, speechIndex
                    )
                    debate.append(row)
                j = text.find(":")
                if j == -1:
                    raise exceptions.couldNotFindNewSpeaker(
                        "could not located : to split new speaker and spoken text", text
                    )
                segmentIndex = 0
                speechIndex += 1
        ...
        if (
            len(debate) > 0
        ):  # there can be no discussion about a topic, see 20% of law proposals
            debate = debate.astype({"speechIndex": "int", "segmentIndex": "int"})
        return debate

    def makeDebateSegmentRow(
        self, text: str, speaker: str, lang: str, segmentIndex: int, speechIndex: int
    ) -> dict:
        return {
    ...
    def mergeTopics(
        ...
        cols = [
            x
            for x in topics.columns
            if (
                (x not in singleColumns)
                and (x != langColumn)
                and (x != matchColumn)
                and (x not in dropColumns)
            )
        ]
        ...
        for code in codes:
            nl = topics.loc[
                (topics[langColumn] == "nl") & (topics[matchColumn] == code)
            ]
            fr = topics.loc[
                (topics[langColumn] == "fr") & (topics[matchColumn] == code)
            ]
            # print("nl:",nl)
            # print("fr:",fr)
