...
class parliamentScraper(scrapy.Spider, parentClass):
    ...
    def loadUselessElementRegex(self):
        path = os.path.join(os.path.dirname(__file__),"operatingData",'uselessElements.csv')
    ...
    def loadChapterHeaders(self):
        ...
        for name in self.chapterHeaders["name"].unique():
            if self.chapterHeaders[self.chapterHeaders["name"] == name].iloc[0]["type"] =="header" :
                regexes = self.chapterHeaders[self.chapterHeaders["name"]==name]
                self.chapterRegexAndName[name] = "|".join(regexes["regex"].tolist()).lower()
            elif self.chapterHeaders[self.chapterHeaders["name"] == name].iloc[0]["type"] =="subHeader":
                regexes = self.chapterHeaders[self.chapterHeaders["name"]==name]
                self.subChapterRegexAndName[name] = "|".join(regexes["regex"].tolist()).lower()
    ...
    def isElementUseless(self,element) -> bool:
        ...
        return ((len(text)==0) or (bool(re.match(self.uselessElementRegex,text,flags=re.IGNORECASE))))
    ...
    def filterOutUselessElements(self,elementList) -> list:
        ...
        for i in range(0,len(elementList)):
            if not self.isElementUseless(elementList[i]):
                out[j] = elementList[i]
                j += 1
        self.status["amountOfElements"],self.status["removedElements"],self.status["originalELements"] = len(elementList)-j,j,len(elementList)
        #print(f"filtered out {len(elementList)-j} elements, leaving {j} elements")
        return out[0:j]

    def isChapterHeader(self,element) -> bool:
        if (bool(re.search("titre1|titel1",self.getClass(element)))):
            return True
        if bool(re.search("titre2|titel2",self.getClass(element))):
            text = self.getInnerText(element)
            #print("text:",text)
            #print("regex result:",bool(re.search("verklaring van de regering|la déclaration du gouvernement",self.removeWeirdCharacters(text),flags=re.IGNORECASE)))
        if (bool(re.search("titre2|titel2",self.getClass(element))) and bool(re.search("\bverklaring van de regering|\bdéclaration du gouvernement",self.getInnerText(element),flags=re.IGNORECASE))):
            return True
        return False
    def getHeaderType(self,element) -> str:
        text = self.removeWeirdCharacters(self.getInnerText(element))
    ...
    def parse(self,response):
        ...
        if len(self.totalElements)<15:
            raise exceptions.tooFewElements(len(self.totalElements),response.request.url)
        self.chapters = self.getChapters(self.totalElements)

        for cs in self.chapterScrapers:
            if cs in self.chapters:
                inp = self.makeInputForChapterScraper(self.totalElements,self.chapters[cs])
    ...
    def save(self,legislature:int,orgName:str,documentNumber:int):
        ...
        if ((not os.path.exists(f"{self.path}/{legislature}")) or (not os.path.isdir(f"{self.path}/{legislature}"))):
            os.mkdir(f"{self.path}/{legislature}")
        
        if ((not os.path.exists(f"{self.path}/{legislature}/{orgName}")) or (not os.path.isdir(f"{self.path}/{legislature}/{orgName}"))):
            os.mkdir(f"{self.path}/{legislature}/{orgName}")
        
        if ((not os.path.exists(f"{self.path}/{legislature}/{orgName}/{documentNumber}")) or (not os.path.isdir(f"{self.path}/{legislature}/{orgName}/{documentNumber}"))):
    ...
    def localToGlobalIndices(self,localIndices:list,ranges:list)->list:
        ...
        for i in range(len(lengths)):
            ...
            #remaining
            globalIndices += [[x[0],x[1],x[2]+shift+ranges[i][0]] for x in localIndices if x[2]<0]
    ...
    def makeBlockIdsGlobal(self):
        ...
        for cs in self.chapterScrapers:
            if cs in self.chapters:
                blocks = [[cs,x[0],x[1]] for x in self.chapterScrapers[cs].getBlockIndices()]
                if len(blocks) > 0:
