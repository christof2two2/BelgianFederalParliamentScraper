...
class parentClass:
    ...
    def getInnerText(self, element) -> str:
        return self.removeWeirdCharacters(
            str("".join(element.css("::text").extract()).strip())
        )
    ...
    def getTag(self, element) -> str:
        return str("".join(element.xpath("name()").extract()).lower().strip())
    ...
    def getLanguageFromElement(self, element) -> str:
        ...
        if out in ["nl", "fr"]:
            return out
        if element.css(":lang(nl)").get() != None:
            return 'nl'
        if element.css(":lang(fr)").get() != None:
            return 'fr'

        try:
            langs = detect_langs(
                self.getInnerText(element)
            )  # fall back to language detection if no lang is specified
            if len(langs) > 0:
                if langs[0].lang in ["nl", "fr"] and langs[0].prob >= 0.4:
    ...
    def removeWeirdCharacters(self, text: str) -> str:
        # parliament uses weird characters like carriage returns and non breaking spaces this functions removes those from a string
        text = re.sub("\u000d", "", text)
        text = re.sub("\u00A0|\u000a", " ", text)
        return text
